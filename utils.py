...
def save_checkpoint(args, data_name, epoch, epochs_since_improvement, encoder_image, chg_filter, encoder_feat, decoder,
...
def logging(model_info, epoch, total_epoch, iter, total_iter, ep, lr, Loss_str, log):
	print_log('{} | Epo: {:02d}/{:02d}, '
		'It: {:04d}/{:04d}, '
		'EP: {:s}, ETA: {:s}, lr {:.6f}, Loss {:s}'
              .format(model_info, epoch, total_epoch, iter, total_iter, convert_secs2time(ep),
                      convert_secs2time(ep / iter * (total_iter * (total_epoch+1 - epoch) - iter)), lr, Loss_str), log)
