...
class CaptionDataset(Dataset):
    ...
    def __init__(self, data_folder, data_name, split, transform=None):
        ...
        # Open hdf5 file where images are stored: num*2*3*256*256
        self.h = h5py.File(os.path.join(data_folder, self.split + '_IMAGES_' + data_name + '.hdf5'), 'r')
        self.imgs = self.h['images']

        # Captions per image: int 5
        self.cpi = self.h.attrs['captions_per_image']

        # Load encoded captions (completely into memory): all captions(img*5) in int list
        with open(os.path.join(data_folder, self.split + '_CAPTIONS_' + data_name + '.json'), 'r') as j:
            self.captions = json.load(j)

        # Load caption lengths (completely into memory): len of all captions
